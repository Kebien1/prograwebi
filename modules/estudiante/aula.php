...
// Permitimos Rol 3 (Estudiante) y Rol 1 (Admin) para que el admin pueda probar
...
// Si es Admin (1), saltamos esta verificación para que pueda entrar libremente
if ($rol_id == 3) {
    ...
    if ($check->rowCount() == 0) { 
        // Si no lo ha comprado, lo mandamos a la página de venta
...
// 5. Determinar la lección actual (Manejo de errores si está vacío)
$clase_actual = null;
if (!empty($lecciones)) {
...
<head>
    ...
    <style>
        body { background-color: #0f111a; color: #e0e0e0; overflow-x: hidden; height: 100vh; display: flex; flex-direction: column; }
        ...
        .player-area { 
            ...
            overflow-y: auto; /* Scroll interno si la descripción es larga */
            position: relative;
        }
        ...
        .lesson-icon { margin-right: 12px; font-size: 1.1rem; }
        ...
        <div>
            <?php if(!empty($lecciones)): ?>
                <span class="badge bg-primary bg-opacity-25 text-primary border border-primary">
                    Lección <?php echo $indice_actual + 1; ?> / <?php echo count($lecciones); ?>
                </span>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="main-layout">
        ...
        <div class="player-area">
            ...
            <?php else: ?>
                <div class="d-flex flex-column align-items-center justify-content-center h-100 text-center p-5">
                    ...
                    <p class="text-muted">El docente aún no ha subido lecciones para este curso.</p>
                    <?php if($rol_id == 1): ?>
                        <a href="../../modules/admin/cursos_lecciones.php?id=<?php echo $id_curso; ?>" class="btn btn-outline-warning mt-3">
                            <i class="bi bi-plus-circle"></i> Agregar Lecciones (Admin)
                        </a>
                    <?php else: ?>
                        <a href="mis_compras.php" class="btn btn-secondary mt-3">Volver a mis cursos</a>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="sidebar-area">
            <div class="sidebar-header d-flex align-items-center justify-content-between">
                ...
                <small class="text-muted"><?php echo count($lecciones); ?> clases</small>
            </div>
                           ...
                           class="lesson-item <?php echo ($index == $indice_actual) ? 'active' : ''; ?>">
                            ...
                            <div class="flex-grow-1">
                                ...
                                <div class="text-muted" style="font-size: 0.75rem;">
                                    <?php echo ($index == $indice_actual) ? 'Reproduciendo...' : 'Lección ' . ($index + 1); ?>
                                </div>
                            </div>
                        </a>
                    <?php endforeach; ?>
